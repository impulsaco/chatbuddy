...
const WordCard = ({ word, translations, menu }) => {
  ...
  const WordTranslations = () => {
    if (translations === true && word.id >= 0) {
      return <Text style={styles.textLight}>{word.translation}</Text>;
    } else if (translations === false && word.id >= 0) {
      return <Text style={styles.textLight}> </Text>;
    } else if (word.id < 0) {
      return null;
    }
  };
  ...
  const Romanization = () => {
    if (word.id >= 0 && word.romanized) {
      return <Text style={styles.textRom}>{word.romanized}</Text>;
    } else {
      return null;
    }
  };
    ...
    <View style={{ width: "100%" }}>
      <TouchableOpacity
        style={[
          styles.wordCard,
          {
            borderColor: borderColor(word.type),
            height: varHeight,
            backgroundColor: backgroundColor(word.type),
          },
        ]}
      >
