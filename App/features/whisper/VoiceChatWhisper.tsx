...
export default ({
  ...
  const selectedLangRef = React.useRef("english");
  selectedLangRef.current = "english";
    ...
    if (Platform.OS === "ios") {
      ...
      axios.post(url, formData, {
        ...
        .then(function (response) {
          ...
          setNewMessage(response.data.text) // Sets the sentence check to be shown
          setCloseVisible(true)
          setTopText('Push to voice chat!')
          console.log("newMessage in Response is ", newMessage)
          /*intervalRef.current = setInterval(
            transcribeInterim,
            transcribeTimeout * 1000
          );*/
      ...
      async function transcribeRecording() {
        ...
        axios({
          ...
          .then(function (response) {
            ...
            setIsTranscribing(false);
            setRecordingDone(false)
            setNewMessage(response.data) // Sets the sentence check to be shown
            setCloseVisible(true)
            console.log("newMessage in Response is ", newMessage)
            /*intervalRef.current = setInterval(
              transcribeInterim,
